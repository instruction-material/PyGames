...
def draw():
    ...
    # draw end screens
        ...
        mod.screen.draw.text(
            "Congratulations,\nYou defeated the alien invasion!",
        ...
        rocket.draw()
        mod.screen.draw.text(
            "Boo!\nYou were defeated by the alien invasion!",
            center=(WIDTH / 2, HEIGHT / 2),
            fontsize=40,
        )
        mod.screen.draw.text(
    ...
    if gameState == "play":
        ...
        for l in lasers:
            ...
            # check if laser collides with alien ship
                ...
                mod.clock.schedule(remove_explosion, 0.3)
                ...
                if alien.health == alien.maxhealth // 2:
                    mod.clock.schedule_interval(alien_shoot, 0.5)
                    mod.clock.schedule_interval(alien_shoot, 0.6)
                elif alien.health == alien.maxhealth // 4:
                    mod.clock.schedule_interval(alien_shoot, 1)
                    mod.clock.schedule_interval(alien_shoot, 0.7)
        ...
        if mod.keyboard.left and rocket.left > 0 and gameState == 'play':
            ...
            mod.keyboard.right and rocket.right < WIDTH and gameState == 'play'
        ):
            rocket.x += 5
...
def on_key_down(key):
    ...
    # reset game
        ...
        mod.clock.unschedule(alien_shoot)
        mod.clock.schedule_interval(alien_shoot, 0.70)
